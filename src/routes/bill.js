...
route.get("/", async (req, res) => {
  //ดูบิล
  const order = await prisma.order.findMany({//ค้นหาเกมทั้งหมด
    where: { userId: req.user.id },
  });
  const billData = await Promise.all(//สร้างloopทั้งหมด
    order.map(async (v) => {
...
route.get("/order/:id", async (req, res) => {
  try {
    ...
    if (!order) {//ถ้าไม่พบorderให้แสดงข้อความบอก
...
route.get("/:id", async (req, res) => {
  try {
    ...
    const bills = await prisma.bill.findUnique({//หาbillผ่านid bill ที่ส่งมา
      where: { id: parseInt(value) },
    });
    if (!bills) {//ถ้าไม่มีบิลให้แสดงว่าไม่พบบิล
...
route.post("/all", async (req, res) => {
  //ดูorder
  const order = await prisma.order.findMany({//ค้นหาorderของuserผ่านtoken
    where: { userId: req.user.id },
  });
  if (order.length === 0) {//ถ้าไม่มีorderให้ส่งข้อมูลกลับมาว่าไม่พบ
...
route.post("/", async (req, res) => {
  ...
  if (!game) {//ถ้าไม่พบเกมให้แจ้งเตือนว่าไม่พบ
  ...
  if (check) {//ถ้าuserมีเกมที่จะซื้ออยู่ให้ส่งกลับเป็นfalse
    return res.send({ check: false });
  }
  await prisma.cart.deleteMany({//ลบเกมที่ซื้อออกจากตะกร้า
    where: {
      userId: req.user.id,
      gameId: game.id,
    },
  ...
  const update = await prisma.game.update({//updateข้อมุลยอดขายเพิ่ม1
    where: {
      id: game.id,
    },
    data: {
  ...
  console.log(value);
  try {
    const bill = await prisma.bill.create({//สร้างบิลและorder
      data: {
        price: game.price,
        Order: {
          create: {
    ...
    console.log(bill);
    return res.send(bill.id);//ส่งbillId กลับไป
...
route.post("/cart", async (req, res) => {
  ...
  try {
    ...
    const billData = await Promise.all(//สร้างbillหลายๆbill
      cart.map(async (v) => {
        const key = crypto.randomBytes(20).toString("hex");//สร้างker game
        const game = await prisma.game.findUnique({//ค้นหาเกมที่จะซื้อ
          where: { id: v.gameId },
        });
    
        await prisma.cart.delete({//ลบเกมที่ซื้อออกจากตะกร้า
          where: { id: v.id },
        });
        //ตรวจสอบว่าuserมีเกมที่จะซื้ออยู่แล้วหรือไม่
        const check = await prisma.order.findFirst({
        ...
        return { id: createdBill.id }; // ส่งค่า id ของบิลที่สร้างขึ้น
      })
    );

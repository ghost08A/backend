...
server.use(//อนุญาตให้มีการเข้าถึงจากทุกๆ โดเมน
  cors({
    origin: "*",
  })
);
...
server.use(//ทำให้อ่านข้อมูลแบบURL-encodedได้
  express.urlencoded({
    extended: true,
  })
);
...
server.use((req, res) => {//ถ้าไม่พบหน้าที่ต้องการให้แสดงกลับไป
  res.status(404).send({
    error: "Not Found",
  });
});

server.listen(PORT, HOST, () => {//เริ่มการทำงานของเซิร์ฟเวอร์
